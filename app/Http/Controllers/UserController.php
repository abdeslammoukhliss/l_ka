    ...
    public function getStudents()
    {
        ...
        foreach($students as $student)
        {
            ...
            foreach($student_courses as $item)
            {
                ...
                $p = DB::select('select c.name as course, m.name as module,p.name as name, gp.deadline, p.description from courses c join modules m on c.id = m.course join projects p on p.module = m.id join groups_projects gp on p.id = gp.project where gp.group = ?',[$item->group_id]);
                $score = 0;
                $rest = null;
                if($pre_score!=null)
                {
